...
import { analyze, createEngine } from "../engine";
...
export async function status(args: string[]): Promise<void> {
  ...
  try {
    ...
    const readinessLabel = result.readiness.ready ? out.green("READY") : out.yellow("NOT READY");

    const boxContent = [
      out.statusLine(`Release Readiness: ${result.readiness.total}/100`, readinessLabel),
      "─".repeat(47),
      out.statusLine(
        "Completeness:",
      ...
      "─".repeat(47),
      out.statusLine("Current version:", out.cyan(result.currentVersion)),
      result.suggestedVersion
        ? out.statusLine("Suggested version:", out.green(result.suggestedVersion))
    ...
    if (result.workUnits.length > 0) {
      ...
      for (const unit of result.workUnits) {
        ...
        const valueColor =
          unit.value === "high" ? out.green : unit.value === "medium" ? out.yellow : out.dim;
